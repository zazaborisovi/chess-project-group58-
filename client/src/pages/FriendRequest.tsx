...
const FriendRequestPage = () => {
  ...
  // Filter requests based on who initiated them
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-12">
        ...
        <header className="space-y-4">
          ...
          <p className="text-base text-slate-400">
            Connect with other players using your unique ID.
        ...
        <div className="flex flex-col gap-4">
            ...
            >
              <div className="flex w-full items-center gap-5 sm:w-auto">
                ...
                <div className="min-w-0">
                  <h3 className="truncate text-lg font-bold text-white">{request.from.username}</h3>
                  <p className="text-[10px] font-mono text-slate-500 uppercase tracking-wider">UID: {request.from._id?.slice(0, 8)}...</p>
                </div>
              </div>
          ...
          {incoming.length === 0 && <p className="text-sm text-slate-600 italic px-2">No pending requests to show.</p>}
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {outgoing.map((request: any, index: number) => (
            ...
            >
              <div className="flex items-center gap-5">
                ...
                <div className="min-w-0">
                  <h3 className="truncate text-base font-bold text-slate-400">{request.to.username}</h3>
                  <p className="text-[10px] font-mono text-slate-600 uppercase">UID: {request.to._id?.slice(0, 8)}...</p>
                </div>
              </div>
          ...
          {outgoing.length === 0 && <p className="text-sm text-slate-600 italic px-2">You haven't sent any invites yet.</p>}
        </div>
      </div>
    </section>
  );
